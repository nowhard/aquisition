module spi_master #(parameter CLK_DIV = 2, DATA_WIDTH = 16)(
  ...
  reg [CLK_DIV_WIDTH-1:0] sck_d, sck_q;
  reg mosi_d, mosi_q;
  ...
  assign mosi = mosi_q;
  assign sck = (/*~*/sck_q[CLK_DIV_WIDTH-1]) & (state_q == TRANSFER);
  ...
  always @(*) begin
    ...
    case (state_q)
      ...
      IDLE: begin
        sck_d = {CLK_DIV_WIDTH{1'b0}};              // reset clock counter
      ...
      WAIT_HALF: begin
        ...
        if (sck_q == {(CLK_DIV_WIDTH-1){1'b1}}) 
		  begin  										// if clock is half full (about to fall)
          sck_d = {CLK_DIV_WIDTH{1'b0}};                        // reset to 0
